...
GetTweet.process = function (data, instance) {
  ...
  for (var i = 0; i < instance.options.howMany; i++) {
    ...
    var thisTweet = fakeAPI.querySelectorAll('.tweet')[i],
    ...
    var thisTweetObj = {
      ...
      isRT: !!thisTweet.querySelectorAll('.retweet-credit').length, // Boolean indicates whether this is a retweet
      link: thisTweet.querySelectorAll('.permalink')[0].href, // Permalink to tweet
      name: userInfo.querySelectorAll('.full-name')[0].innerText.replace(/^\s+|\s+$/g,''), // Full name of user
      ...
      time: Date.parse(thisTweet.querySelectorAll('.permalink')[0].getAttribute('data-datetime')), // Number of milliseconds since January 1, 1970, 00:00:00 UTC
      user: userInfo.querySelectorAll('.p-nickname')[0].innerText // User's screen name
    }
